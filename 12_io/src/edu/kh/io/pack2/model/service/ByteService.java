...
public class ByteService {
	 ...
	 *   모든 것을 입/출력 할 수 있음
	 *   (단, 통로가 좁다보니 속도가 느림, 1byte 초과인 문자같은 경우 깨질 우려가 있음)
	 *   
	 * */
	...
	public void fileByteOutput() {
		...
		try {
			...
			// StringBulider(비동기)/StringBuffer(동기)
			...
			sb.append("선생님 저 정말 공부 열심히 할게요~\n");
			sb.append("진짜로!!정말로!!화이팅!!\n");
			
			// 출력방법 1 : 한 글자씩(2byte) 파일로 출력
		    ...
		    */
			...
			fos.write(content.getBytes()); // byte[]로 만들어 변환하고
			//                                내부적으로 문자 인코딩을 적용하여 한글도 깨지지 않고 출력
			...
			fos.flush(); // FileOutputStream에서는 flush() 선택사항
	...
	public void fileByteInput() {
		...
		try {
			...
			// :/io_test/20251014/노래가사.txt(지정된 파일이 없습니다.)
