...
// 자식타입 참조변수 = 자식객체주소
		...
		// Tesla 객체가 Car(부모) 객체로 변함
		// -> 업 캐스팅 (up casting)
		
		Car car3 = new Spark(); //오류발생 x
		// Spark 객체를 참조하는 변수의 타입이 Car(부모)이기 때문에
		// Spark 객체가 Car(부모) 객체로 변함
		// -> 업 캐스팅 (up casting)
		...
		// The method setBatteryCapacity() is undefined for the type Car
		...
		// The method setDiscountOffer() is undefined for the type Car
		
		// car2,car3는 부모타입 참조변수(Car)이기 때문에
		// Tesla,Spark와 같은 자식객체의 고유필드, 메서드 사용불가
		// 컴파일러가 Car라고 인식하고 있기 때문..
		
		// 2) 다형성을 이용한 객체배열
		Car arr[] = new Car[3];
		// 각 요소가 Car 객체만 들어올 수 있는 객체배열 선언 및 할당
		...
		// 상속 + 다형성
		// 상속 특징 : Car 상속 클래스는 모드 getEngine() 메서드를 이용할 수 있다.
	...
	public void printCar(Car temp) {
		...
		// 부모타입 참조변수 = 자식타입객체
		// -> 다형성의 업캐스팅 모양과 똑같다
		// -> Car 와 Spark/Tesla는 상속관계이기 때문에
		// temp라는 Car(부모타입) 매개변수에 Car,Tesla,Spark의 주소가 참조가능!!
	...
	public void ex4() {
		...
		Car c1 = new Tesla();
		// Tesla 객체를 만들어서 Car타입 참조변수 c1에 담음 
		// ->업캐스팅
		
		// Spark s1 = (Spark)c1; // c1을 관계없는 Spark로 다운캐스팅하려함
		// 다운캐스팅 잘못입력해도 컴파일 에러가 발생 안함
		// -> 실행 사 런타임에러 발생
		// ClassCastException : 형변환 예외
		
		// 해결방법 : istanceof 와 같이 사용하면 된다!
