...
public class StudentService {
	...
	public StudentService() { // 기본 생성자
		...
		studentList.add(new Student("박주희",24,"서울시 서대문구",'F',100));
	}
	
	
	// new ArrayList() : 기본생성자 -> 기본 크기 10칸짜리 리스트 생성
	// 하지만 리스트의 크기는 늘었다 줄었다 하기 때문에 큰 의미는 없음
	
	public void ex() { // List 테스트용 메서드
		
		// List.add(Object e) : 리스트의 객체를 추가
	 ...
	 * 
	 * @author 조윤아(redccc1234 @ gamil.com)
	 */
	...
	public void selectAll() {
		...
		/* 일반 for문
		for(int i = 0 ; i < studentList.size() ; i++) { // lenght (x)
			System.out.println(studentList.get(i)); 
		}*/
		
		// 향상된 for문
		// -컬렉션, 배열의 모든 요소를 순차적으로 반복접근 할 수 있다
		int index = 0;
	...
	*   
	*  -수정할 때
	* 1. index번째에 저장된 학생정보를 수정 전에 한 번 출력
	* 2. 이름 나이 사는 곳 성별 점수 수정할 값 입력받기
	* 3. 수정이 완료되었다면,"000의 정보가 변경되었습니다" 출력
	...
	public String updateStudent() {
		...
		int index = sc.nextInt();
			...
			// E List.set(int index , E e) : 주어진 index번 째에 주어진 객체로 덮어쓰기 (수정함)
			//                               기존의 객체를 반환함
	...
	public String removeStudent() {
		...
		int index = sc.nextInt();
			...
			// E List.remove(int index) : 리스트에서 index번째 요소를 제거
			// 이때 제거된 요소 반환됨.
			// * List 중간에 비어있는 인덱스가 없게 하기 위해서
			// remove() 동작 시 뒤쪽 요소를 한 칸씩 자동으로 당겨오는 행위를 하고 있음
			if(result == 'Y') {
	...
	private void sortByName() {
		...
		Collections.sort(studentList,new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				// 이름 비교
				// name은 String 형이라 compareTo로 비교
				// String.compareTo() : 자바에서 객체를 비교하는 메서드
				// String 이 Comparable을 상속받아재정의 해둔 compareTo() 메서드를 이용하는 것
