...
public class MemberService {
	...
	public void displaymMenu() {
		...
		}while(menuNum!=0);// menuNum 0이 되면 종료
		
	}
	
	// 회원 가입 기능
	public String signUp() {// 정의만 한 것임 -> 호출이라는 과정이 필요함!!
		...
		if(memberPw.equals(memberPwCheck)) {//일치하는 경우
			...
			// 회원가입
			// 새로운 회원 객체 생성
			// -> Member클래스 이용해서 새로운 객체를 생성
			// -> 매개변수 생성자 이용
			memberInfo = new Member(memberId,memberPw,memberName,memberAge);
			// 회원가입한 정보를 가진 객체의 주소값을
			// memberInfo 필드변수에 참조시킴
			// -> 회원가입 진행 후 memberInfo는 Null이 아니다.
			return "회원가입 성공🙌🙌";
	...
	// 로그인 기능
	public String login() {// 정의만 한 것임 -> 호출이라는 과정이 필요함!!
		...
		// 회원가입 정보(memberInfo 가 참조하는  member 객체)에
		// 저장된 아이디,비밀번호가 
		...
		if(memberId.equals(memberInfo.getMemberId()) && memberPw.equals(memberInfo.getMemberPw())) {
			...
			// == loginMember 에 memberInfo가 참조하고 있는 객체의
			loginMember = memberInfo; // 얕은 복사가 일어남
			return loginMember.getMemberName()+ "님 환경합니다!!"; 
	...
	public String selectMember() {
		// -> 조회하려면? 로그인을 했는지 확인
		// -> 로그인 했다면?
		// 현재 로그인 한 사람의 정보를 저장한 객채에 정보 얻어오기(이름,아이디,나이)
	...
	public int updateMember() {
		...
		// 2-2) 비밀번호를 입력받아서 현재 로그인 한 회원의 비밀번호와 일치하는지 확인
		System.out.print("비밀번호 확인 : ");
		String inputPw = sc.next();
		
		if(inputPw.equals(loginMember.getMemberPw())) {
			// -> 비밀번호가 일치하는 경우
			// 로그인 한 회원의 이름과 나이 정보를 
