...
public class ConditionExample {// 기능 제고용 클래스
	...
	public void ex4() { 
		...
		//9~11 가을
		//1~12 사이가 아닐 때 " 해당하는 계절이 없습니다" 출력
	...
	public void ex8() {
		...
		// 나이 O , 키 O : "탑승 가능"
			...
			if( (age < 0) || (age > 100) ){
				...
				if((age<12) && (height >= 140.0)) {// 나이 부적절, 키 적절
						result ="키는 적절하나, 나이는 적절치 않음";
					}else if( (age >=12) && (height < 140.0) ) { //나이 적절, 키 부적절
						result ="나이는 적절하나, 키가 적절치 않음";
					}else if( (age <12) && ( height < 140.0)) { // 둘 다 부적절
	...
	public void ex10() { 
		...
		// 10으로 나눈 몫이 
		// 10,9 라면 A
		// 8 이면 B , 7 이면 C, 6 이면 D, 그 외 (5~0) F 출력
		...
		default : System.out.println("잘못된 점수"); break;
		}
	}
}
