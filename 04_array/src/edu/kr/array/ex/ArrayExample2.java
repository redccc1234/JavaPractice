...
public class ArrayExample2 {
	 ...
	 public void createLottoNumber() {
		 // 로또 번호 생성기
		 // 배열을 이용한 중복 데이터를 제거하고 정렬하는 
		 // 1. 1~45 사이 중복되지 않은 난수를 6개 생성
		 // 2. 생성된 난수를 오름차순으로 정렬
		 // [5,11,13,18,24,25]
		 
		 // 1) 정수 6개를 저장할 배열이 선언 및 할당
		 int lotto[] = new int[6];
		 ...
		 for(int i = 0 ; i < lotto.length ; i++ ) {
			 ...
			 // 5) 중복 검사를 위한 for문을 작성하자
			 for(int x = 0 ; x < i ; x++) {
				
				 // 6) 현재 생성된 난수와 같은 수가 이전 요소에 있는지 검사
				 if(random == lotto[x]) {
					i--;
					// i가 1씩 증가할 때마다 난수가 하나 생성된
		 ...
		 // 7) 정렬
		 // 선택,삽입,버블,퀵 등등
		 // --> 자바가 정렬 방법을 미리 만들어서 제공하고 있습니다
		 // Arrays.sort(배열명) : 배열 내 요소값들이 오름차순으로 정렬
