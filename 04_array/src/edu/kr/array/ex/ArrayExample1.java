...
public class ArrayExample1 {
	...
	public void ex1() {
			...
			arr = new int[3];
			// new " new 연산자 " 라고 하며
			//      heap 메모리 공간에 새로운 공간( 배열, 객체 )을 할당
					
			// int[3] : int 자료형 변수 3개를 하나의 묶음으로 나타내는 배열		
			
			// new int[3] : heap 영역에 int 형 변수 3칸짜리 int[] 생성(할당)
			...
			arr[1] = 50;
			arr[2] = 100;
			//arr[3] = 500; 런타임에러 발생 -> 배열 범위를 벗어남 예외 발생
	...
	public void ex2() {
		...
		// 그 공간에 arr2이라는 이름 부여
		// heap 메모리 영역에 int5칸 배열을 생성하고
		// 0번 인덱스부터 각 요소에 1~5까지 정수값을 대입
		// int[] 주소값을 arr2참조변수가 참조함.
	...
	public void ex5() {
		//점심 메뉴 뽑기 프로그램
		String arr[] = {"김밥","서브웨이","국밥","짜장면","햄버거","백반"};
	...
	public void ex6() {
		...
		// 입력받은 정수가 배열에 있는지 없는지 확인(boolean)
	...
	public void ex8() {
		...
		// 1. 문자열을 입력 받아 한 글자씩 잘라내어
		// char 배열에 순서대로 저장
		// 2. 문자 하나를 입력 받아 일치하는 문자가 char배열에 몇 개 존재하는지 확인
		// 3. 단, 일치하는 문자가 없을 경우에는 " 존재하지 않습니다" 출력
		
		// 사용해야 하는 기능,기술
		// 1) 배열 검색
		// 2) 문자열의 길이를 구하기 : String.length()
		// ex) "hello".length() -> 5
		...
		// char 배열에 순서대로 저장
		// 1-1) 입력받은 문자열 길이만큼 char 배열 선언 및 할당하기
			char arr[] = new char[input.length()];
			
			// 1-2) 입력한 문자열을 한글자씩 잘라내어 char 배열의 모든 요소에 순서대로 값 대입
			for(int i = 0 ; i < arr.length ; i++) {
		...
		// "e".charAt(0); -> 'e'
		// -> Scanner에는 char형태를 입력받을 수 있는 메서드가 없다
		// -> String 으로 우선 입력받고 입력받은 String에서
		// charAt(index)메서드를 이용하여 해당 index번째 존재하는  
		// 문자 하나를 떼어 char자료형 형태로 변환
